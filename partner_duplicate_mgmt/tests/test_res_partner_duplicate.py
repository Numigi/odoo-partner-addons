...
class PartnerDuplicateCase(common.SavepointCase):
    ...
    def setUpClass(cls):
        ...
        cls.env = Environment(cls.env.cr, cls.env.ref("base.user_demo").id, {}).context(
            dict(cls.env.context, test_queue_job_no_delay=True)
        )
...
class TestResPartnerDuplicate(PartnerDuplicateCase):
    ...
    def test_when_partner_is_archived_duplicates_left_are_resolved(self):
        ...
        dup1 = self.env["res.partner.duplicate"].search(
            [("partner_1_id", "=", partner_1.id), ("partner_2_id", "=", partner_2.id)]
        )
        dup2 = self.env["res.partner.duplicate"].search(
            [("partner_1_id", "=", partner_1.id), ("partner_2_id", "=", partner_3.id)]
        )
        dup3 = self.env["res.partner.duplicate"].search(
            [("partner_1_id", "=", partner_2.id), ("partner_2_id", "=", partner_3.id)]
        )
    ...
    def _get_archived_partner_message(self, partner):
        return partner.message_ids.filtered(lambda m: "Merged into" in (m.body or ""))

    def _get_preserved_partner_message(self, partner):
        return partner.message_ids.filtered(lambda m: "Merged with" in (m.body or ""))

    def _get_duplicate_partner_message(self, partner):
        return partner.message_ids.filtered(lambda m: "Duplicate" in (m.body or ""))
